...
let startGyro = () => {
  ...
  try {
    ...
    gyroscope.onreading = () => {
      ...
      sloshed =
        Math.round((passed / (failed + passed)) * 100) < drunkThresh
          ? true
          : false;
      if (dashboard) {
        document.getElementById("passRate").innerHTML =
          Math.round((passed / (failed + passed)) * 100) + "%";
        document.getElementById("drunk").innerHTML = sloshed
          ? "UR SLOSHED AS!!"
          : "Nah u good.";
    ...
    gyroscope.onerror = (event) => {
      ...
      if (dashboard)
        (document.getElementById("error").innerHTML = event.error.name),
          event.error.message;
      error = true;
    };
  } catch (error) {
